...
pub struct DirEntry {
    pub name: [u8; 11],        // 8.3 format: "HELLO   TXT" for "HELLO.TXT" (no dot stored)
    pub attr: u8,              // File attributes: 0x10=directory, 0x20=archive (regular file)
    pub size: u32,             // File size in bytes (0 for directories)
    pub start_cluster: u32,    // First cluster number where file data begins (clusters are like blocks)
}
...
pub struct Fat32Info {
    pub bytes_per_sector: u16,      // Usually 512 bytes (one SD card block)
    pub sectors_per_cluster: u8,    // How many sectors form one cluster (cluster = allocation unit)
    pub reserved_sectors: u16,      // Sectors before first FAT (usually contains boot sector)
    pub num_fats: u8,               // Number of FAT copies (usually 2 for redundancy)
    pub fat_size_sectors: u32,      // Size of one FAT in sectors
    pub root_dir_cluster: u32,      // Starting cluster of root directory (/)
    pub total_sectors: u32,         // Total sectors on the volume
    // Calculated fields (derived from above):
    pub fat_start_lba: u32,         // LBA (Logical Block Address) where first FAT starts
    pub data_start_lba: u32,        // LBA where actual file data starts
}
...
impl Fat32Info {
    ...
    pub fn parse(boot_sector: &[u8; 512]) -> Result<Self, &'static str> {
...
pub async fn sd_init<SPI, CS>(spi: &mut SPI, cs: &mut CS) -> Result<bool, &'static str>
where
    SPI: SpiBus<u8>,
    CS: OutputPin,
{
...
pub async fn sd_read_block<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
...
pub async fn sd_write_block<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    lba: u32,              // Logical Block Address (sector number)
...
pub async fn fat32_write_file<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    filename: &str,
...
pub async fn fat32_find_file<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
...
pub async fn fat32_create_directory<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    parent_cluster: u32,
...
pub async fn fat32_add_dir_entry<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    dir_cluster: u32,
...
pub async fn fat32_list_directory<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    dir_cluster: u32,
...
pub async fn fat32_navigate_path<'a, SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    path: &'a str,
...
pub async fn fat32_write_file_at_path<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    path: &str,
...
pub async fn fat32_write_file_in_dir<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
...
pub async fn fat32_read_file_at_path<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    path: &str,
...
pub async fn fat32_read_file_complete<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    start_cluster: u32,
...
pub async fn fat32_delete_file<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    dir_cluster: u32,
...
pub async fn fat32_delete_file_at_path<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    path: &str,
...
pub async fn fat32_delete_directory<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    parent_cluster: u32,
...
pub async fn fat32_delete_directory_at_path<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    path: &str,
...
pub async fn fat32_rename_file<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    dir_cluster: u32,
...
pub async fn fat32_find_directory_by_path<SPI, CS>(
    spi: &mut SPI,
    cs: &mut CS,
    fat_info: &Fat32Info,
    path: &str,
